 ...
 *
 * The same paths as the console's `CatMark`, copied rather than imported —
 * this app has to build and deploy without the product's package graph, and one
 * SVG is a cheaper dependency than a shared UI package that exists for it.
 * If the drawing changes, it changes in both places.
 */
export function Mark({ className = "size-8" }: { className?: string }) {
  return (
    <svg viewBox="0 0 32 32" className={className} role="img" aria-label="drymem">
      <title>drymem</title>
      <path
      ...
      <path
        ...
        fill="var(--background)"
      />
      <path d="M16 22.6 L14.7 24.3 H17.3 Z" fill="var(--brand)" opacity="0.5" />
    </svg>
  );
}
