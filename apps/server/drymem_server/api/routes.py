...
async def memory_context(
    ...
    episodes = await service.context(project_key=project_key, limit=limit)
    return ContextResponse(
        ...
        episodes=[
            EpisodeOut(
                ...
                author=e.metadata.author if e.metadata else None,
                scope=e.metadata.scope if e.metadata else "private",
            )
            for e in episodes
        ],
    )
